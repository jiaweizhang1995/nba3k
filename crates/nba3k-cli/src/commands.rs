...
fn cmd_new(app: &mut AppState, args: NewArgs) -> Result<()> {
    ...
    // M18-A: seed three owner-mandate goals for the user team based on
    // roster strength. Runs after team assignment so `roster_for_team`
    // sees the freshly imported seed roster.
    let user_team_id = current_state(app)?.user_team;
    auto_generate_mandates(app, season, user_team_id)?;
...
fn cmd_season_advance(app: &mut AppState, as_json: bool) -> Result<()> {
    ...
    // M18-A: re-seed owner mandates for the new season. Runs after
    // progression + draft so `avg_top8_ovr` reflects the new roster (rookies
    // assigned, retirees removed, OVR deltas applied).
    auto_generate_mandates(app, next_season, new_state.user_team)?;
...
// M18-A — owner mandate.
//
// Three season goals are auto-seeded at season start (cmd_new + the start
// of each new season inside cmd_season_advance) keyed by the user team's
// roster strength. `cmd_mandate` walks them, computes per-goal pass-rate
// vs the live save state, and once the regular season is finished folds
// them into a single weighted A/B/C/D/F grade.
// ----------------------------------------------------------------------

/// Mandate kind strings persisted in the `mandate` table. Pinned as
/// constants so the auto-gen heuristic and the progress reader can never
/// drift (e.g. "make_playoff" vs "make_playoffs").
const MANDATE_WINS: &str = "wins";
const MANDATE_PLAYOFFS: &str = "make_playoffs";
const MANDATE_DEVELOP_TO: &str = "develop_to";
const MANDATE_CHAMPION: &str = "champion";
const MANDATE_LOTTERY_TOP3: &str = "lottery_top3";

/// Average OVR of the team's top-8 active players. Returns 0 for empty
/// rosters so the rebuild bucket gets picked rather than panicking.
fn avg_top8_ovr(app: &mut AppState, team: TeamId) -> Result<u32> {
    let roster = app.store()?.roster_for_team(team)?;
    if roster.is_empty() {
        return Ok(0);
    }
    let mut ovrs: Vec<u32> = roster.iter().map(|p| p.overall as u32).collect();
    ovrs.sort_unstable_by(|a, b| b.cmp(a));
    let n = ovrs.len().min(8);
    let sum: u32 = ovrs.iter().take(n).sum();
    Ok(sum / n as u32)
}

/// Auto-seed three mandates for `team` for `season` based on its roster
/// strength bucket. UPSERTs so callers can invoke this idempotently at
/// season-rollover without producing duplicate goals.
fn auto_generate_mandates(
    app: &mut AppState,
    season: SeasonId,
    team: TeamId,
) -> Result<()> {
    let avg = avg_top8_ovr(app, team)?;
    // (kind, target, weight) tuples. Weights inside each bucket sum to 1.0
    // so the weighted-pass-rate computed in cmd_mandate doesn't need to
    // be re-normalized at render time.
    let goals: &[(&str, i32, f32)] = if avg >= 84 {
        &[
            (MANDATE_PLAYOFFS, 1, 0.30),
            (MANDATE_WINS, 50, 0.40),
            (MANDATE_CHAMPION, 1, 0.30),
        ]
    } else if avg >= 78 {
        &[
            (MANDATE_PLAYOFFS, 1, 0.30),
            (MANDATE_WINS, 42, 0.50),
            (MANDATE_DEVELOP_TO, 85, 0.20),
        ]
    } else {
        &[
            (MANDATE_WINS, 28, 0.40),
            (MANDATE_DEVELOP_TO, 78, 0.30),
            (MANDATE_LOTTERY_TOP3, 1, 0.30),
        ]
    };
    let store = app.store()?;
    for (kind, target, weight) in goals {
        store.record_mandate(season, team, kind, *target, *weight)?;
    }
    Ok(())
}

/// Map a weighted pass-rate to the A/B/C/D/F letter grade. Thresholds are
/// pinned by the store-layer test in `tests/mandate.rs` — keep the two in
/// sync if you tune them.
fn mandate_grade(score: f32) -> &'static str {
    if score >= 0.85 {
        "A"
    } else if score >= 0.70 {
        "B"
...
/// Per-goal progress snapshot: `current` is the live measurement (wins,
/// max OVR, 0/1 boolean), `pass` is `current / target` clamped to [0, 1].
struct MandateProgress {
    kind: String,
    target: i32,
    weight: f32,
    current: i32,
...
fn compute_progress(
    ...
    season: SeasonId,
    user_team: TeamId,
    rows: &[nba3k_store::MandateRow],
) -> Result<Vec<MandateProgress>> {
    // Precomputes — every kind needs at most one of these and running them
    // eagerly keeps the per-row branch trivial.
    let teams = app.store()?.list_teams()?;
    let mut standings = Standings::new(&teams);
    for g in app.store()?.read_games(season)? {
        if !g.is_playoffs {
            standings.record_game_result(&g);
        }
    }
    ...
    let standings_wins = standings
        .records
        .get(&user_team)
        .map(|r| r.wins as i32)
        .unwrap_or(0);

    let bracket = nba3k_season::generate_bracket(&standings, season);
    let in_playoff_bracket = bracket
        .r1
        .iter()
        .any(|s| s.home == user_team || s.away == user_team);

    let max_roster_ovr = app
        .store()?
        .roster_for_team(user_team)?
        .iter()
    ...
    let series_rows = app.store()?.read_series(season)?;
    let is_champion = series_rows
        .iter()
        .find(|s| s.round == nba3k_season::PlayoffRound::Finals as u8)
        .map(|s| {
            let winner = if s.home_wins == 4 { s.home_team } else { s.away_team };
            winner == user_team
        })
        .unwrap_or(false);

    // Lottery top-3 only resolves once at least one season has been simmed
    // — skip the precompute for any mandate that doesn't ask for it so an
    // empty draft order on a fresh save doesn't bail.
    let needs_lottery = rows.iter().any(|r| r.kind == MANDATE_LOTTERY_TOP3);
    let lottery_top3 = if needs_lottery {
        let order = draft_order(app)?;
        order.iter().take(3).any(|t| *t == user_team)
    } else {
        false
    };
    ...
    for row in rows {
        let (current, pass) = match row.kind.as_str() {
            MANDATE_WINS => {
                let target = row.target.max(1) as f32;
                let pass = (standings_wins as f32 / target).clamp(0.0, 1.0);
                (standings_wins, pass)
            }
            MANDATE_PLAYOFFS => {
                let cur = if in_playoff_bracket { 1 } else { 0 };
                (cur, cur as f32)
            }
            MANDATE_DEVELOP_TO => {
                let cur = max_roster_ovr;
                let pass = if cur >= row.target { 1.0 } else { 0.0 };
                (cur, pass)
            }
            MANDATE_CHAMPION => {
                let cur = if is_champion { 1 } else { 0 };
                (cur, cur as f32)
            }
            MANDATE_LOTTERY_TOP3 => {
                let cur = if lottery_top3 { 1 } else { 0 };
                (cur, cur as f32)
            }
            _ => (0, 0.0),
        };
        out.push(MandateProgress {
            kind: row.kind.clone(),
            target: row.target,
            weight: row.weight,
            current,
...
fn render_mandate_text(
    ...
    println!("Season {} mandate ({}):", season.0, team_label);
    if progress.is_empty() {
        println!("  (no mandate set)");
        return;
    }
    for p in progress {
        let status = if p.pass >= 1.0 {
            "PASS"
        } else if p.pass > 0.0 {
            "..  "
        } else {
        ...
        println!(
            "  [{}] {:<14} target={:<3}  current={:<3}  weight={:.2}  pass={:.2}",
            status, p.kind, p.target, p.current, p.weight, p.pass
        );
    }
    if let Some(g) = grade {
        println!("  grade: {} (score={:.2})", g, score);
    } else {
        println!("  grade: pending (regular season not yet complete)");
    }
}

fn cmd_mandate(app: &mut AppState, season: Option<u16>, as_json: bool) -> Result<()> {
    let state = current_state(app)?;
    let target_season = season.map(SeasonId).unwrap_or(state.season);
    let user_team = state.user_team;
    let team_label = app
        .store()?
        .team_abbrev(user_team)?
    ...
    let rows = app.store()?.read_mandates(target_season, user_team)?;
    let progress = compute_progress(app, target_season, user_team, &rows)?;

    let total_weight: f32 = progress.iter().map(|p| p.weight).sum();
    let weighted_pass: f32 = progress.iter().map(|p| p.weight * p.pass).sum();
    ...
    let regular_done = regular_complete(app, target_season)?;
    let grade = if regular_done && !progress.is_empty() {
        Some(mandate_grade(score))
    } else {
        None
    };

    if as_json {
        let goals: Vec<_> = progress
            .iter()
            .map(|p| {
                json!({
                    "kind": p.kind,
        ...
        println!(
            "{}",
            serde_json::to_string_pretty(&json!({
                "season": target_season.0,
                "team": team_label,
                "goals": goals,
                "score": score,
                "grade": grade,
                "regular_complete": regular_done,
            }))?
        );
    } else {
        render_mandate_text(target_season, &team_label, &progress, grade, score);
    }
    Ok(())
}
